...
NSString* UIKit_GetURLScheme(NSString* identifier)
{
    NSString* ret = @"";
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([name isEqualToString: identifier] && [schemes count] == 1) {
            ret = [schemes objectAtIndex: 0];
            return ret;
        }
...
void UIKit_ResumeMixableAudio()
{
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
    if (audioSession.category == AVAudioSessionCategoryPlayback) {
        if (!(audioSession.categoryOptions & AVAudioSessionCategoryOptionMixWithOthers)) {
            NSLog(@"UIKit_ResumeMixableAudio: current is playback, 1, set mixable audio");
			NSError* error1 = nil;
            ...
            NSLog(@"UIKit_ResumeMixableAudio: current is playback, 2, do nothing");
        }
    } else {
        NSLog(@"UIKit_ResumeMixableAudio: current isn't playback, do nothing");
    }
}
...
void UIKit_ShowStatusBar(SDL_bool visible)
{
	if (visible) {
...
int UIKit_GetStatusBarHeight()
{
    CGRect rectStatus = [[UIApplication sharedApplication] statusBarFrame];
    return rectStatus.size.height * UIKit_GetDpiScale();
}
...
const char* UIKit_ApnsGetToken()
{
	return [apns_token UTF8String];
}
