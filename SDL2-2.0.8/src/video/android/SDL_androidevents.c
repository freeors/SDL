...
extern void Android_PumpBleDid(SDL_bool foreground);
...
static SDL_bool fls_nonblock_pump = SDL_FALSE;
void Android_SetNonblockPump(SDL_bool nonblock)
{
	fls_nonblock_pump = nonblock;
}

void Android_PumpEvents_Nonblock()
{
    static int isPaused = 0;
    /* No polling necessary */
	Android_PumpBleDid(SDL_TRUE);
...
{
	if (fls_nonblock_pump) {
		Android_PumpEvents_Nonblock();
		return;
	}
