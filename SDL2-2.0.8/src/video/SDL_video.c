...
extern void SDL_ThirdInit(void);
extern void SDL_ThirdQuit(void);
extern void SDL_PeripheralInit(void);
extern void SDL_PeripheralQuit(void);
...
extern int UIKit_MicrophonePermission(void);
extern void UIKit_SetKeyboardReturn(SDL_Window* window, int type);

typedef void(*fn_acceleration_callback)(double x, double y, double z);
extern void UIKit_StartAccelerometer(SDL_Window* window, const int hz, fn_acceleration_callback fn, SDL_bool gyroscope);
extern void UIKit_StopAccelerometer(SDL_Window* window, SDL_bool gyroscope);
extern void UIKit_IExplore(const char* url);
extern void UIKit_EnableBatteryMonitoring(SDL_bool enable);
extern int UIKit_GetBatteryLevel(void);
extern const char* UIKit_ApnsGetToken(void);
#endif
...
SDL_SetWindowData(SDL_Window * window, const char *name, void *userdata)
{
	...
	if (!window) {
		if (len == 14 && !SDL_memcmp(name, "battery_enable", len)) {
			SDL_bool enable = (int)userdata ? SDL_TRUE : SDL_FALSE;
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_EnableBatteryMonitoring(enable);
#endif

		}
		else if (len == 13 && !SDL_memcmp(name, "battery_level", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			return (void*)UIKit_GetBatteryLevel();
#endif
			return (void*)(100);

		}
		else if (len == 8 && !SDL_memcmp(name, "iexplore", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_IExplore((const char*)userdata);
#endif
		}
		else if (len == 21 && !SDL_memcmp(name, "microphone_permission", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			return (void*)UIKit_MicrophonePermission();
#else
			return (void*)1;
#endif
		}
	...
	if (len >= 2) {
		if (len >= 3 && name[0] == '#' && name[1] == '#' && (name[2] == 'G' || name[2] == 'A')) {
			SDL_bool gyroscope = name[2] == 'G';
#if defined(__IPHONEOS__) && __IPHONEOS__
			if (len > 3) {
				int hz = SDL_atoi(name + 3);
				UIKit_StartAccelerometer(window, hz, (fn_acceleration_callback)userdata, gyroscope);
			}
			else {
				UIKit_StopAccelerometer(window, gyroscope);
			}
#endif
			return NULL;
		}
		else if (len >= 10 && !SDL_memcmp(name, "keyboard_", 9)) {
			int type = SDL_atoi(name + 9);
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_SetKeyboardReturn(window, type);
#endif
			return NULL;
		}
	}
...
int SDL_GetDpiScale()
{
	// Normal SDL_GetDpiScale should be called before INIT_VIDEO. At that moment, _this is null. 

#if SDL_VIDEO_DRIVER_WINDOWS
	extern int WIN_GetDpiScale();
	return WIN_GetDpiScale();
#endif

#if SDL_VIDEO_DRIVER_UIKIT
	extern int UIKit_GetDpiScale(void);
	return UIKit_GetDpiScale();
#endif

#if SDL_VIDEO_DRIVER_ANDROID
	extern int Android_GetDpiScale();
	return Android_GetDpiScale();
#endif
	return 1;
}

int SDL_GetStatusBarHeight()
{
#if SDL_VIDEO_DRIVER_WINDOWS
	return 0;
#endif
...
#if SDL_VIDEO_DRIVER_UIKIT
	extern int UIKit_GetStatusBarHeight(void);
	return UIKit_GetStatusBarHeight();
#endif

#if SDL_VIDEO_DRIVER_ANDROID
	extern int Android_GetStatusBarHeight();
	return Android_GetStatusBarHeight();
#endif
}

void SDL_ShowStatusBar(SDL_bool visible)
{
#if SDL_VIDEO_DRIVER_WINDOWS
	return;
#endif
...
#if SDL_VIDEO_DRIVER_UIKIT
    extern void UIKit_ShowStatusBar(SDL_bool visible);
    UIKit_ShowStatusBar(visible);
#endif

#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_ShowStatusBar(SDL_bool visible);
	Android_ShowStatusBar(visible);
#endif
}

void SDL_EnableStatusBar(SDL_bool enable)
{
#if SDL_VIDEO_DRIVER_WINDOWS
	return;
#endif
...
#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_EnableStatusBar(SDL_bool enable);
	Android_EnableStatusBar(enable);
#endif
}

void SDL_SetStatusBarColor(SDL_bool white_font)
{
#if SDL_VIDEO_DRIVER_WINDOWS
	return;
#endif
...
#if SDL_VIDEO_DRIVER_UIKIT
    extern void UIKit_SetStatusBarStyle(SDL_bool light);
    UIKit_SetStatusBarStyle(white_font);
#endif

#if SDL_VIDEO_DRIVER_ANDROID
	// Now android doesn't support change font color.
#endif
}

void SDL_TurnOnScreen(SDL_bool on)
{
#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_TurnOnScreen(SDL_bool on);
	Android_TurnOnScreen(on);
#endif
}

void SDL_Reboot()
{
	if (sdl_apphandlers.reboot_handler != NULL) {
		if (!sdl_apphandlers.reboot_handler(sdl_apphandlers.reboot_param)) {
			return;
		}
	}
#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_JNI_Reboot();
	Android_JNI_Reboot();
#endif
}

void SDL_SetAppType(int type)
{
#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_JNI_SetAppType(int type);
	Android_JNI_SetAppType(type);
#endif
}

void SDL_SetNonblockPump(SDL_bool nonblock)
{
#if SDL_VIDEO_DRIVER_ANDROID
	extern void Android_SetNonblockPump(SDL_bool nonblock);
	Android_SetNonblockPump(nonblock);
#endif
}

SDL_AppHandlers sdl_apphandlers = {0};

void SDL_SetAppHandlers(SDL_AppHandlers* handlers)
{
	SDL_memcpy(&sdl_apphandlers, handlers, sizeof(SDL_AppHandlers));
}
