...
int WIN_GetDpiScale()
{
	// this function must call before INIT_VIDEO. WIN_InitModes require actual screen size.
	...
	GetProcessDpiAwareness = (HRESULT(WINAPI *)(HANDLE, PROCESS_DPI_AWARENESS*)) SDL_LoadFunction(shcoreDLL, "GetProcessDpiAwareness");
	GetDpiForMonitor = (HRESULT(WINAPI *)(HMONITOR, MONITOR_DPI_TYPE, UINT *, UINT *)) SDL_LoadFunction(shcoreDLL, "GetDpiForMonitor");
	if (!GetProcessDpiAwareness || !SetProcessDpiAwareness || !GetDpiForMonitor) {
		return 1;
	}
	PROCESS_DPI_AWARENESS using_awareness;
	HANDLE hProcess;
	hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, GetCurrentProcessId());
	GetProcessDpiAwareness(hProcess, &using_awareness);

	if (using_awareness == PROCESS_DPI_UNAWARE) {
		// As long as dpi-aware, use PROCESS_PER_MONITOR_DPI_AWARE. Don't use PROCESS_SYSTEM_DPI_AWARE.
