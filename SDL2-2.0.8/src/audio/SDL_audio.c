...
{
	if (!get_audio_device(device->id)) {
		// this audio session has been closed, do nothing.
		// may occure in capture.
		return;
	}
...
SDL_PauseAudioDevice(SDL_AudioDeviceID devid, int pause_on)
{
    SDL_AudioDevice *device = get_audio_device(devid);
    if (device) {
		int pause_on2 = pause_on? 1 : 0;
		if (pause_on2 == SDL_AtomicGet(&device->paused)) {
			return;
		}
        current_audio.impl.LockDevice(device);
		if (current_audio.impl.PauseDevice) {
			current_audio.impl.PauseDevice(device, pause_on2);
		}
