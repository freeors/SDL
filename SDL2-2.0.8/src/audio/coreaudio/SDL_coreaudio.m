...
*/
			// Fix! when foreground, should set to Ambient, but at least iOS9.0.2 work error!
			category = AVAudioSessionCategoryPlayback;
        }

		SDL_bool mixable = SDL_TRUE;
		if (open_capture_devices || (didbackground && this->iscapture)) {
			// 1: start capture. 2: stop capture and when in background.
			mixable = SDL_FALSE;
		}
		BOOL ret;
		if (mixable) {
		    ret = [session setCategory:category withOptions: AVAudioSessionCategoryOptionMixWithOthers error: &err];
    	} else {
			ret = [session setCategory:category error: &err];
		}
...
outputCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef inBuffer)
{
	if (didbackground && sdl_apphandlers.background_handler) {
        sdl_apphandlers.background_handler(SDL_TRUE, sdl_apphandlers.background_param);
    }
...
static void COREAUDIO_PauseDevice(_THIS, int pause_on)
{
	if (pause_on) {
		...
		OSStatus result = AudioQueueStart(this->hidden->audioQueue, NULL);
		// CHECK_RESULT("AudioQueueStart");
		if (result != noErr) {
			// SDL_SetError("CoreAudio error (%s): %d", msg, (int) result);
			return;
...
COREAUDIO_Init(SDL_AudioDriverImpl * impl)
{
	...
	impl->PauseDevice = COREAUDIO_PauseDevice;
