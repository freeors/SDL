...
#define ADJUST_VOLUME_S64B(s, v, min, max) {	\
	if (v >= 0) {	\
		Sint64 s1 = (s*v)/SDL_MIX_MAXVOLUME;		\
		if (s1 < (min)) {	\
			s1 = (min);		\
		} else if (s1 > (max)) {	\
			s1 = (max);		\
...
#define ADJUST_VOLUME_U32B(s, v, max) {	\
	if (v >= 0) {	\
		int s1 = (s*v)/SDL_MIX_MAXVOLUME;		\
		if (s1 > (max)) {	\
			s1 = (max);		\
    ...
    case AUDIO_U8:
        {
            ...
            while (len--) {
				...
				ADJUST_VOLUME_U32B(src_sample, volume, 255);
    ...
    case AUDIO_S8:
        {
            ...
            while (len--) {
				...
				ADJUST_VOLUME_S32B(src_sample, volume, -127, 127);
    ...
    case AUDIO_S16LSB:
        {
            ...
            while (len--) {
				...
				ADJUST_VOLUME_S32B(src1, volume, -32767, 32767);
                src2 = ((dst[1]) << 8 | dst[0]);
                src += 2;
    ...
    case AUDIO_U16LSB:
        {
            ...
            while (len--) {
                src1 = ((src[1]) << 8 | src[0]);
                // ADJUST_VOLUME(src1, volume);
				ADJUST_VOLUME_U32B(src1, volume, 65535);
                src2 = ((dst[1]) << 8 | dst[0]);
                src += 2;
                dst_sample = src1 + src2;
                if (dst_sample > max_audioval) {
    ...
    case AUDIO_S32LSB:
        {
            ...
            while (len--) {
                ...
                src32++;
                // ADJUST_VOLUME(src1, volume);
				ADJUST_VOLUME_S64B(src1, volume, -2147483647, 2147483647);
