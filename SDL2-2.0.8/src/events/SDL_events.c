...
SDL_SendAppEvent(SDL_EventType eventType)
{
	...
	if (eventType == SDL_APP_DIDENTERBACKGROUND) {
		didbackground = 1;
	} else if (eventType == SDL_APP_DIDENTERFOREGROUND) {
		didbackground = 0;
	}

	if (eventType != SDL_APP_LOWMEMORY && sdl_apphandlers.app_event_handler) {
		sdl_apphandlers.app_event_handler(eventType, sdl_apphandlers.app_event_param);
		return 1;
	}
