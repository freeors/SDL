...
static void (*get_mix_data_hook)(Uint8*, int, void*) = NULL;
static void* get_mix_data_contex = NULL;
void Mix_HookGetMixData(void (*get_mix_data)(Uint8*, int, void*), void* contex)
{
    SDL_LockAudio();
	get_mix_data_hook = get_mix_data;
	get_mix_data_contex = contex;
	if (get_mix_data_hook) {
		SDL_PauseAudio(0);
	}
    SDL_UnlockAudio();
}
...
static void mix_channels(void *udata, Uint8 *stream, int len)
{
	...
	if (!get_mix_data_hook) {
		...
		get_mix_data_hook(stream, len, get_mix_data_contex);
	}
	...
	if (!require_xmit && !persist_xmit_audio && !get_mix_data_hook) {
